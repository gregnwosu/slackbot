...
def convo(user_input, name="Dave") -> str:
    chat = ChatOpenAI(model_name="gpt-3.5-turbo", temperature=1)
    ...
    client = pinecone.Client(api_key=memory.PINECONE_API_KEY)

    chain = ConversationChain(llm=chat, prompt=chat_prompt,  memory=pinecone.PineconeMemory(client=client))
    return chain.run(user_input=user_input, signature=signature, name=name)
