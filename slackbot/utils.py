...
async def cache_channel_memory(
    channel_id: str,
    channel_memory_cache: aioredis.Redis,
...
async def get_memory_from_cache(
    channel_id: str, channel_memory_cache: aioredis.Redis
) -> Optional[ConversationSummaryBufferMemory]:
    channel_memory_json: Optional[str] = await channel_memory_cache.get(
        f"channel_memory:{channel_id}"
    )
...
async def get_memory_for_channel(channel_id: str) -> ConversationSummaryBufferMemory:
    async with get_cache() as channel_memory_cache:
        channel_memory: ConversationSummaryBufferMemory = await get_memory_from_cache(
            channel_id=channel_id, channel_memory_cache=channel_memory_cache
        )
        if not channel_memory:
            channel_memory = ConversationSummaryBufferMemory(llm=LLM.GPT4.value)
            await cache_channel_memory(
                channel_id=channel_id,
                channel_memory_cache=channel_memory_cache,
